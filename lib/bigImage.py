...
class tiledImage:
    ...
    def __init__(self, width, height):
        ...
        self.num_images = nearest_square(num_pixels/self.max_num_pixels)

        self._images = []
        for image in range(self.num_images):
            self._images.append(numpy.zeros((1080,1920,3),dtype=numpy.uint8))
    ...
    def add_text(self, text, center):
        ...
        self.text[imNum].append([text, center])
    ...
    def write(self):
        for i in trange(len(self._images), desc="Writing images", position=0):
            for edge in tqdm(self.edges[i], desc="edges", position=1, leave=False):
                self._images[i] = cv2.line(self._images[i], edges[0], edges[1], (100,100,100), thickness=2)
            for circle in tqdm(self.circle[i], desc="circles", position=2, leave=False):
                self._images[i] = cv2.circle(self._images[i], circle[0], circle[1], circle[2], thickness=-1)
            for text in tqdm(self.text[i], desc="text", position=3, leave=False):
                self._images[i] = cv2.putText(self._images[i], text[0], text[1], cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255))
            
            cv2.imwrite("images/image_{}.png".format(i),self._images[i])#, [cv2.IMWRITE_JPEG_QUALITY, 50])
    
    def position_to_image(self, org):
        """
        pos: tuple, (x,y)
